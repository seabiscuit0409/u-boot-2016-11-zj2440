...
static const struct amd_flash_info jedec_table[] = {
	...
	{	/*added by zhj*/
		...
		.NumEraseRegions= 4,
		.regions	= {
			ERASEINFO(16*1024, 1),			/*16K*/	
			ERASEINFO(8*1024, 2),			/*8K*/
			ERASEINFO(32*1024, 1),			/*32K*/
			ERASEINFO(64*1024, 31),			/*64K*/
		}
	},
};
...
int jedec_flash_match(flash_info_t *info, ulong base)
{
	...
	for (i = 0; i < ARRAY_SIZE(jedec_table); i++) {
		debug("jedec_table_mfr_id = %x\n", jedec_table[i].mfr_id & mask);
		debug("jedec_table_dev_id = %x\n", jedec_table[i].dev_id & mask);
		debug("info->manufacturer_id = %x\n", info->manufacturer_id & mask);
		debug("info->device_id = %x\n", info->device_id & mask);
